...
import { getProductBadge, getProductMerit, getProductSold, getProductTitle } from "../lib/productLocale";
...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const merit = getProductMerit(product, language);
        ...
        {product.colors && (
          <div className="absolute bottom-2 left-2">
            <span className="bg-black/60 text-white text-[10px] px-2 py-0.5 rounded-sm">
              {product.colors}+ {language === "zh" ? "色" : "Colors"}
            </span>
          </div>
        )}
      </div>
      <div className="p-2.5">
        <p className="text-[12px] leading-4 text-gray-800 line-clamp-2 mb-1.5">{title}</p>
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-1.5">
            <span className="text-[#0A1931] font-bold text-[14px] font-outfit">{formatCurrency(product.price, currency)}</span>
            <span className="text-gray-400 text-[11px] line-through">{formatCurrency(product.originalPrice, currency)}</span>
          </div>
        ...
        <div className="flex items-center gap-1 mb-1.5">
          ...
          <span className="text-[#F97316] text-[11px] font-medium">{sold}</span>
        </div>
        {merit && (
        ...
        <div className="flex flex-wrap gap-1">
          ...
          <span className="text-[10px] bg-green-100 text-green-800 px-2 py-0.5 rounded-full flex items-center gap-0.5">
            ...
            {language === "zh" ? "工厂直发" : "Factory Direct"}
          </span>
        </div>
      </div>
    </Link>
