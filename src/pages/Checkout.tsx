            ...
            {step === "payment" && (
              <div className="bg-white rounded-xl p-6">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button onClick={() => setStep("info")} className="w-full sm:w-auto px-6 py-3 border border-gray-200 rounded-xl text-sm text-gray-600 hover:bg-gray-50 transition-colors">Back</button>
                  <button onClick={handleHostedCheckout} disabled={isSubmitting} className="flex-1 bg-[#0A1931] hover:bg-[#061021] disabled:opacity-40 text-white font-outfit font-bold py-3.5 px-4 rounded-xl transition-colors flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 leading-tight">
                    {isSubmitting ? <><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg><span>Redirecting to Paystack...</span></> : <><svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg><span className="text-sm">Pay {formatCurrency(subtotal, currency)}</span><span className="text-xs opacity-80 hidden sm:inline">with Paystack</span><span className="text-xs opacity-80 sm:hidden">Paystack Secure</span></>}
                  </button>
                </div>
